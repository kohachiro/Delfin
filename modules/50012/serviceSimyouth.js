...
function assign( response, putData )  {
	...
	var length = putData.readUInt16BE(2);
...
function getPlayerProto(player) {
	var playerProto = new Data.Player({"id":player.id,"name":player.name,"gem":player.gem,"gold":player.gold,"level":player.level,"exp":player.exp});
	return playerProto;
}
/**
 * get player list
